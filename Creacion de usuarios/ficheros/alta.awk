...
BEGIN{
		...
		dominio="OU=Formacion Profesional,DC=gonzalez,DC=com";
...
}
{
		...
		usuario=arreglar(tolower(substr($1,1,1)substr($2,1,3)substr($3,1,3)))
			...
			print "dsadd user -upn "usuario"@gonzalez.com \"CN="nombreordenado","unidad"\" -samid "usuario" -disabled no -pwd $M"clave"a# -mustchpwd yes -memberof \""grupo"\" -fn \""nombre"\" -ln \""apellido1" "apellido2"\" -display \""nombrecompleto"\" -q" > fichero
